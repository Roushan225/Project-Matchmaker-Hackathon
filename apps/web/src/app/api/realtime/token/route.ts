...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "A project is required." }, { status: 400 });
    ...
    const token = await new SignJWT({ projectId: parsed.data.projectId }).setProtectedHeader({ alg: "HS256" }).setSubject(user.id).setIssuedAt().setExpirationTime("5m").sign(encoder.encode(secret));
