...
export default async function ProjectDetailPage({
  ...
  let persistedProject: Awaited<ReturnType<typeof findProjectBySlug>> = null;

  if (!project) {
    try {
      const publishedProject = await findProjectBySlug(slug);
      if (publishedProject) {
        persistedProject = publishedProject;
  ...
  const projectStatus = persistedProject?.status ?? "recruiting";
  const isOwner = persistedProject?.ownerId === user.id;
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] space-y-5">
          ...
          <div className="mt-8 flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              <p className="text-sm font-semibold uppercase tracking-[0.16em] text-indigo-200">
                {project.category} · {projectStatus}
              </p>
              <h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-white md:text-6xl">
                {project.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-8 text-indigo-100/70">
                {project.description}
              </p>
            </div>
            {isOwner && persistedProject ? (
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Metric
          label="Team members"
          value={`${project.memberCount}/${project.maxTeamSize}`}
        />
        <Metric label="Open roles" value={openSlots} />
        <Metric label="Required skills" value={project.requiredSkills.length} />
      ...
      <section className="grid gap-5 xl:grid-cols-[1.3fr_1fr]">
        <div className="rounded-2xl border border-white/12 bg-white/[0.07] p-6 shadow-xl shadow-indigo-950/15 backdrop-blur">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-lg font-semibold text-white">Existing team</p>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {project.members.map((member) => (
              <Link
                ...
                className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.05] p-4 transition hover:border-white/25 hover:bg-white/[0.1]"
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-indigo-100/55">
                    {member.role}
                  </p>
                </div>
                <span className="ml-auto text-indigo-100/50">→</span>
              </Link>
            ))}
          </div>
        </div>
        <div className="rounded-2xl border border-white/12 bg-white/[0.07] p-6 shadow-xl shadow-indigo-950/15 backdrop-blur">
          ...
          <p className="mt-1 text-sm text-indigo-100/55">
            Bring one of these strengths to the team.
          </p>
          ...
          <div className="mt-7 border-t border-white/10 pt-5">
            ...
            <p className="mt-2 text-3xl font-semibold text-white">
              {openSlots} role{openSlots === 1 ? "" : "s"}
            </p>
            <p className="mt-1 text-sm text-indigo-100/60">
              The team is actively looking for contributors.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-white/12 bg-white/[0.07] shadow-xl shadow-indigo-950/15 backdrop-blur">
        <div className="grid divide-y divide-white/10 md:grid-cols-3 md:divide-x md:divide-y-0">
          <div className="p-6">
            ...
            <p className="mt-1 text-sm leading-6 text-indigo-100/60">
              Check whether your skills match the team’s current needs.
            </p>
          </div>
          <div className="p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-100/50">
            ...
            <p className="mt-2 font-medium text-white">Apply with context</p>
            <p className="mt-1 text-sm leading-6 text-indigo-100/60">
              Explain what you can build and the role you want to own.
            </p>
          </div>
          <div className="p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-100/50">
            ...
            <p className="mt-2 font-medium text-white">Join the private hub</p>
            <p className="mt-1 text-sm leading-6 text-indigo-100/60">
              Accepted members get a focused workspace for the project.
            </p>
          </div>
        </div>
      </section>
