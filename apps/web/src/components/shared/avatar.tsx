...
export function Avatar({ name, image, size = "md" }: AvatarProps) {
  ...
  return <span className={`${dimensions} inline-flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-200 to-violet-300 font-bold text-indigo-950`}>{name.slice(0, 1).toUpperCase()}</span>;
}
