...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-[1440px] space-y-5">
    ...
    <section className="rounded-2xl border border-white/12 bg-white/[0.07] p-6 shadow-xl shadow-indigo-950/15 backdrop-blur"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-lg font-semibold text-white">Contribution activity</p><p className="mt-1 text-sm text-indigo-100/55">GitHub commit and contribution activity from the last 12 months.</p></div>{github?.syncedAt && <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1 text-xs text-indigo-100/60">Synced {github.syncedAt.toLocaleDateString()}</span>}</div><GitHubContributionGraph totalContributions={github?.contributionCalendar?.totalContributions ?? 0} days={github?.contributionCalendar?.days ?? []} /></section>

    <section className="rounded-2xl border border-white/12 bg-white/[0.07] shadow-xl shadow-indigo-950/15 backdrop-blur"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 px-6 py-5"><div><p className="text-lg font-semibold text-white">GitHub repositories</p><p className="mt-1 text-sm text-indigo-100/55">Public repositories with current activity and commit progress.</p></div><span className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs text-indigo-100/70">{github?.repositories.length ?? 0} synced</span></div>{github?.repositories.length ? <div className="divide-y divide-white/10">{github.repositories.map((repository) => <a key={repository.id} href={repository.url} target="_blank" rel="noreferrer" className="group grid gap-4 px-6 py-5 transition hover:bg-white/[0.06] md:grid-cols-[minmax(0,1.4fr)_minmax(240px,1fr)_auto] md:items-center"><div className="min-w-0"><p className="font-medium text-white group-hover:text-indigo-100">{repository.name}</p><p className="mt-1 truncate text-sm text-indigo-100/55">{repository.description || "No repository description."}</p><div className="mt-3 flex items-center gap-3"><div className="h-1.5 min-w-24 flex-1 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-violet-300 to-fuchsia-300" style={{ width: `${Math.min(100, (repository.recentCommitCount ?? 0) * 10)}%` }} /></div><span className="whitespace-nowrap text-xs text-indigo-100/55">{repository.recentCommitCount ?? 0} commits · 30d</span></div></div><div className="flex flex-wrap gap-2">{repository.primaryLanguage && <span className="rounded-full border border-white/10 bg-white/[0.06] px-2.5 py-1 text-xs text-indigo-100/75">{repository.primaryLanguage}</span>}{repository.topics.slice(0, 2).map((topic) => <span key={topic} className="rounded-full border border-white/10 bg-white/[0.06] px-2.5 py-1 text-xs text-indigo-100/60">{topic}</span>)}</div><div className="flex items-center gap-4 text-sm text-indigo-100/60"><span>★ {repository.stars}</span><span>⑂ {repository.forks}</span><span className="text-white/70">Open ↗</span></div></a>)}</div> : <div className="px-6 py-12 text-center text-sm text-indigo-100/55">No GitHub repository data has been synced yet.</div>}</section>
  </div>;
}
