...
import { listWorkspaceProjectsForUser } from "@/server/repositories/projects";
import { SidebarNav } from "./sidebar-nav";

export async function AppShell({ children, userId, userName }: { children: React.ReactNode; userId: string; userName: string }) {
  const hubProjects = await listWorkspaceProjectsForUser(userId);
  ...
  return (
    <div className="min-h-screen bg-[#09052a] md:grid md:grid-cols-[250px_1fr]">
      <aside className="border-b border-white/10 bg-[#0e073d]/85 p-5 backdrop-blur md:min-h-screen md:border-b-0 md:border-r">
        ...
        <SidebarNav hubProjects={hubProjects} />
